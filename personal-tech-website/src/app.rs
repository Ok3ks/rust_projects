        ...
        <Router>
            <main>
                <Routes fallback=|| "Not found.">
                    //Main Routes
                    <Route path=path!("/write") view=Write/>
                    <Route path=path!("/editor/:id") view=Editor/>
...
fn HomePage() -> impl IntoView {
    view! {
        ...
        <div class="container" style="padding: 4rem 1rem;">
            ...
            <A href="#articles">"Thoughts"</A  > //use Navigate to move to another page on click
        </div>
    </header>
    </head>
...
fn Editor() -> impl IntoView {
    ...
    let title_element = NodeRef::<Textarea>::new();
    let content_element = NodeRef::<Textarea>::new();

    let (title, set_title) = signal("Title".to_string());
    let (content, set_content) = signal("Content".to_string());

    let on_submit = move |ev: SubmitEvent| {
        println!("Submit Triggered");
          ev.prevent_default();
        let value_title = title_element.get().expect("<input> to exist").value();
        let value_content = content_element.get().expect("<input> to exist").value();

        set_title.set(value_title.clone());
        set_content.set(value_content.clone());

        let current_article = Article {
            id: id.parse::<i16>().expect("REASON"),
            title: &value_title,
            text_content: &value_content,
            is_draft: true,
            is_published: false,
        };
    ...
    view! {
        ...
        //escaping form input
        <form class="writing-form" method="post" on:submit=on_submit>
        <textarea
        class="title-input"
        placeholder="Title"
        name="title"
...
fn Write() -> impl IntoView {
    ...
    view! {
        ...
        <div>
            <Drafts/>
            <button on:click= move |_| { navigate(&format!("/editor/{0}",  &id), Default::default()) } > "New" </button> //redirect to Editor with id
        </div>
    }
}
